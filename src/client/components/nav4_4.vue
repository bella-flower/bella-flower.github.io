    ...
    <div class="content">
      <div class="header">
        ...
        <div class="rightbtn">
          ...
          <el-button icon="el-icon-circle-check-outline" plain size="small" :open="showmodel1()" @click="permissionVisible()" style="margin-right: 5px">{{$t('nav4_4.buttonGroup.perBtn')}}</el-button>
          <el-button  size="small" icon="el-icon-circle-close-outline" @click="removePerVisible()" style="margin-right: 5px">{{$t('nav4_4.buttonGroup.removePer')}}</el-button>
      ...
      <!--授权框-->
      <el-dialog :title="$t('nav4_4.perdialog.title')" :visible.sync="PermissionVisible" ref="perdialog"  width="680px" :before-close="closePermission">
        <el-form  :model="newdialog" ref="perform">
          <el-form-item :label="$t('nav4_4.perdialog.label_1')"  :label-width="newdialog.formLabelWidth">
            <el-input v-model="permissionDialog.file"  auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item :label="$t('nav4_4.perdialog.label_2')"  :label-width="newdialog.formLabelWidth">
            <el-input :placeholder="$t('nav2_1.select.placeholder')"  v-model="filterText" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item :label="$t('nav4_4.perdialog.label_3')"  style="height:220px;" :label-width="newdialog.formLabelWidth">
            <div class="area">
              <div class="tesearea">
                <el-tree :data="permissionDialog.userInfo" :props="permissionDialog.defaultProps" show-checkbox node-key="id" ref="tree" :expand-on-click-node="true" :filter-node-method="filterNode" ></el-tree>
              </div>
            </div>
          </el-form-item>
          <el-form-item :label="$t('nav4_4.perdialog.label_4')" :label-width="newdialog.formLabelWidth">
            <el-checkbox-group v-model="permissionDialog.checkList">
              <el-checkbox label="R">{{$t('nav4_4.perdialog.radio_1')}}</el-checkbox>
              <el-checkbox label="W">{{$t('nav4_4.perdialog.radio_2')}}</el-checkbox>
              <el-checkbox label="E">{{$t('nav4_4.perdialog.radio_3')}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="cleanPermission">{{$t('nav2_1.buttonGroup.cancelBtn')}}</el-button>
          <el-button type="primary"  @click="newSubmit('newform')">{{$t('nav2_1.buttonGroup.sureBtn')}}</el-button>
        </div>
      </el-dialog>
      <!--取消授权-->
      <el-dialog :title="$t('nav4_4.removePerdialog.title')" :visible.sync="removePermission" width="480px" id="el-dialog1">
        <div style="text-align: center;">
          <img src="../../../static/img/promot.png" alt="">
          <div>
            <span>{{$t('nav4_4.removePerdialog.content')}}</span>
            <span>{{rePermission.filename}}</span>?
          </div>
        </div>
        <div slot="footer" class="dialog-footer">
              <el-button @click="removePermission = false">{{$t('nav3_2.dialog.cancel')}}</el-button>
              <el-button type="primary">{{$t('nav3_2.dialog.Determine')}}</el-button>
        </div>
      </el-dialog>
    </div>
  ...
  export default {
    data () {   /*组件的data对象是data()函数不是data*/
      return {
        ...
        PermissionVisible:false,/*授权*/
        removePermission:false,/*取消授权*/
        filterText:'',/*搜索*/
        newdialog:{
          new:'',
    ...
    methods: {
      ...
      removePerVisible:function(){
        if(this.jobDataActive.length<=0){
          this.$message.error(this.$t('rvcpinfo.nosellect'));
        }else {
          console.log(this.jobDataActive);
          this.removePermission = true;
          this.rePermission.filename=this.jobDataActive[0].name;
        }
      },/*移除授权*/
      ...
      cleanPermission:function(){
        this.$refs['perform'].resetFields();
        this.PermissionVisible = false
      },/*清除授权表单数据*/
      closePermission:function(done){
