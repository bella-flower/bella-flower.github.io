    ...
    <div class="content">
      <div class="header">
        ...
        <div class="rightbtn">
          ...
          <el-button size="small" icon="el-icon-edit-outline" @click="dialogHouVisible=true">{{$t('nav4_2.buttonGroup.houBtn')}}</el-button>
        </div>
      </div>
      ...
      <!--复制和移动-->
      <el-dialog  :title="$t('nav4_1.detailInfo.movedialog.title')" :visible.sync="detailInfo.dialogMoveVisible" width="880px" :before-close="closedialog3">
        <el-container>
          ...
          <el-container>
            ...
            <el-footer class="movefoot" style="height:81px;">
              <div style="width:570px">
                <el-form :model="detailInfo.moveDialog.moveFoot"  label-width="120px" ref="form3">
              ...
              <div style="width:100px;">
                <el-button type="primary"  size="small" @click="cleanform3()" style="margin-bottom: 5px;width:80px;">{{$t('nav3_2.dialog.Determine')}}</el-button>
      ...
      <!--后处理框-->
      <el-dialog :title="$t('nav4_2.houdialog.title')" :visible.sync="dialogHouVisible" ref="houdialog"  width="480px" :before-close="closeHoudialog">
        <el-form  :model="houdialog" ref="houform">
          <el-form-item :label="$t('nav4_2.houdialog.label_1')" :label-width="newdialog.formLabelWidth">
            <el-select v-model="houdialog.resouluValue" :placeholder="$t('nav2_1.select.placeholder')" size="small">
              <el-option  v-for="(item, index) in houdialog.resolution" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item :label="$t('nav4_2.houdialog.label_2')"  :label-width="newdialog.formLabelWidth">
            <el-select v-model="houdialog.commandValue" :placeholder="$t('nav2_1.select.placeholder')" size="small">
              <el-option  v-for="(item, index) in houdialog.command" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="cleanHouform">{{$t('nav2_1.buttonGroup.cancelBtn')}}</el-button>
          <el-button type="primary"  @click="newSubmit('yasuoform')">{{$t('nav2_1.buttonGroup.sureBtn')}}</el-button>
        </div>
      </el-dialog>
      <!--备份-->
      <el-dialog :title="$t('nav4_2.backupDialog.title')" :visible.sync="detailInfo.backupVisible" ref="backupdialog"  width="680px" :before-close="closeBackup">
        <el-form  :model="detailInfo.backupDialog" ref="backupform">
          ...
          <el-form-item :label="$t('nav4_2.backupDialog.label_5')"  style="height:220px;" :label-width="newdialog.formLabelWidth">
            <div class="area">
              <el-input :placeholder="$t('nav4_2.placeholder')"  v-model="filterText" auto-complete="off" style="width:100%;padding:10px;box-sizing:border-box;" ></el-input>
              <div class="treearea">
  ...
  export default {
    ...
    methods: {
      ...
      cleanform3(){
        ...
        this.detailInfo.dialogMoveVisible = false;
      },//清除表单数据
      closedialog3(done){
        this.cleanform3();
        done();
      ...
      cleanHouform:function(){
        this.$refs['houform'].resetFields();
        this.dialogHouVisible= false
      },/*关闭后处理对话框回调*/
      closeHoudialog:function(done){
        this.cleanHouform();
        done();
      },/*关闭后处理表单数据*/
